...
from tensorflow.keras.layers import Input, Conv2D, MaxPooling2D, UpSampling2D, concatenate, Conv2DTranspose, BatchNormalization, \
    Dropout, Lambda, Activation, Add, Multiply
...
def attention_block(skip_connection, gating_signal, inter_channels):
    """Applies spatial attention gating on the skip connection."""
    theta_x = Conv2D(inter_channels, (1, 1), padding='same')(skip_connection)
    theta_x = BatchNormalization()(theta_x)

    phi_g = Conv2D(inter_channels, (1, 1), padding='same')(gating_signal)
    phi_g = BatchNormalization()(phi_g)

    combined = Add()([theta_x, phi_g])
    combined = Activation('relu')(combined)

    psi = Conv2D(1, (1, 1), padding='same')(combined)
...
def multi_unet_model(n_classes=4, IMG_HEIGHT=256, IMG_WIDTH=256, IMG_CHANNELS=1):
    ...
    # Expansive path with attention gated skip connections
    u6 = Conv2DTranspose(64, (2, 2), strides=(2, 2), padding='same')(c5)
    attn_c4 = attention_block(c4, u6, 64)
    ...
    attn_c3 = attention_block(c3, u7, 32)
    ...
    attn_c2 = attention_block(c2, u8, 16)
    ...
    attn_c1 = attention_block(c1, u9, 8)
    ...
    # Multi-scale feature fusion for enhanced decoding
    decoder_features = [c6, c7, c8, c9]
    scale_factors = [8, 4, 2, 1]
    multi_scale_logits = []
    for feature, scale in zip(decoder_features, scale_factors):
        logits = Conv2D(n_classes, (1, 1), padding='same')(feature)
        if scale > 1:
            logits = UpSampling2D(size=(scale, scale))(logits)
        multi_scale_logits.append(logits)

    fused = concatenate(multi_scale_logits, axis=-1)
    outputs = Conv2D(n_classes, (1, 1), activation='softmax')(fused)
